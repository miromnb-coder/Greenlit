...
-- The application switches to these org-scoped tables after this migration.
...
create table if not exists messages (
  ...
  lead_id uuid not null references leads(id) on delete cascade, provider text not null,
  provider_message_id text not null, provider_thread_id text, direction text not null,
  body text not null default '', subject text not null default '', received_at timestamptz not null default now(),
  created_at timestamptz not null default now(), unique (organization_id, provider, provider_message_id)
);
create table if not exists jobs_new (
  id uuid primary key, organization_id uuid not null references organizations(id) on delete cascade,
  type text not null, lead_id uuid references leads(id) on delete cascade, status text not null,
  detail text not null, idempotency_key text, created_at timestamptz not null default now(), finished_at timestamptz
);

-- Existing rows are assigned to a migration workspace. A real signed-in user is linked
-- automatically by lib/auth.ts on first request. Do not expose this workspace to clients.
do $$
declare migration_org uuid;
begin
  select id into migration_org from organizations order by created_at limit 1;
  if migration_org is null then
    insert into organizations(name) values ('Migrated Greenlit workspace') returning id into migration_org;
  end if;
  ...
  insert into org_playbooks(organization_id, data)
    select migration_org, data from playbook where id = 'default'
    on conflict (organization_id) do nothing;
  insert into org_connections(organization_id, gmail, hubspot_token)
    select migration_org, gmail, hubspot_token from connections where id = 'default'
    on conflict (organization_id) do nothing;
end $$;
