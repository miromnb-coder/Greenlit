...
The production path is now organization-scoped and human-approved.
...
- `WEBHOOK_SECRET` — long random secret; there is no development fallback in production
- `APP_URL`
...
If an existing deployment contains migrated data, assign the migration workspace to the intended first user in `organization_members` before using the application.

## Security model

- Every signed-in user gets an organization membership on first authenticated request.
- Server-side data access always filters by the authenticated organization.
